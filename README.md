...
The analysis uses the mcPHASES dataset, which combines:

* Hormonal measurements (LH, estrogen)
* Physiological measurements from wearable devices

  * Resting heart rate (RHR)
  * Wrist temperature
  * Movement-related variables
* Self-reported cycle information

Only a subset of the available variables was used in this project.

# Main Workflow

1. Data extraction and cleaning (ETL)
2. Subject-level time series construction
3. Physiological alignment using the LH peak
4. Standardization and preprocessing
5. DTW distance computation
6. Hierarchical clustering and K-Medoids clustering
7. Cluster evaluation and visualization
8. Comparison against known cycle phases

# Main Result

Despite testing multiple preprocessing strategies, linkage methods, and DTW-based approaches, no clear cluster structure matching the menstrual cycle phases was observed in the selected subset of subjects and variables.

This negative result is still valuable because it highlights the difficulty of recovering menstrual phases from unsupervised clustering alone and provides a reproducible baseline for future work.

# Limitations

* Small number of subjects after filtering and preprocessing.
* Missing values and unequal recording frequencies.
* Only a subset of the available physiological signals was analyzed.
* Cycle phases may not be sufficiently separable using unsupervised methods alone.
* DTW similarity may capture subject-specific behavior rather than phase-specific behavior.
